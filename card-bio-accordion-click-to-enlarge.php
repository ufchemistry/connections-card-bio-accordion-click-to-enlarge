...
/**
 * @package    Connections
 * @subpackage Template : Bio Card Accordion
 ...
 *
 * @wordpress-plugin
 * Plugin Name:       Connections Bio Card Accordion - Template
...
function getHtmlTagArray_cbacte($htmlTag) {
	...
	return($htmlTagArray);
} ##END function getHtmlTagArray_cbacte($imgTag)

function cn_remove_list_no_result_message2() {
    remove_action( 'cn_list_no_results', array( 'cnTemplatePart', 'noResults' ), 10 );
}

add_action( 'plugins_loaded', 'cn_remove_list_no_result_message2', 12 );


if ( ! class_exists( 'CN_Bio_Card_Accordion_Enlarge_Template' ) ) {

	class CN_Bio_Card_Accordion_Enlarge_Template {

		public static function register() {

			$atts = array(
				'class'       => 'CN_Bio_Card_Accordion_Enlarge_Template',
				'name'        => 'Bio Entry Accordion Card Accordion-click-to-enlarge',
				'slug'        => 'card-bio-accordion-click-to-enlarge',
				'type'        => 'all',
				'version'     => '2.0.1',
				'author'      => 'Steven A. Zahm',
...
*/
			?>
<!-- START card-bio-accordion-click-to-enlarge -->
<?php
global $thisCatFound;
...
if ($photo) {
	...
	##/wp-content/uploads/sites/50/connections-images/hsuan-chun-lin/Hsuan-ChunLin-180x225-1-51a90cd22f2b8543554f28ba15814381.jpg 1x/  /span  /
...
list($span1, $span2, $logo, $endspan1, $endspan2) = explode("><", $logo);
...
/*
$photoTagArray = getHtmlTagArray_cbacte($photo);
$srcset = $photoTagArray['srcset'];
...
$photoalt = $photoTagArray['alt'];
*/
echo '<span class="cn-image-style"><span style="display: block; max-width: 100%; width: 180px"><a target="_blank" href="' . $photoUrl . '">
' . $logo . '</a></span></span>';
...
<!-- END card-bio-accordion-click-to-enlarge -->
	...
	add_action( 'cn_register_template', array( 'CN_Bio_Card_Accordion_Enlarge_Template', 'register' ) );
}
